...
import theshaybi.androidinvertormonitor.IMessageListener;
...
public class PocketPos {
    ...
    public static final byte FRAME_STATUS_RESPONSE = 0x53;
...
*/
    ...
    private static int asciiToInt(byte[] buf, int offset, int length) {
        if (buf == null || offset < 0 || length < 1 || length > 10 || buf.length < (offset + length))
            return -1;
...
*/
    ...
    public static byte[] FramePack(byte typeFrame, byte[] srcBuff, int srcOffset, int srcLen) {
        if (typeFrame == FRAME_NACK || typeFrame == FRAME_ACK || typeFrame == FRAME_ENQ || typeFrame == FRAME_EOT) {
        ...
        if (typeFrame == FRAME_PHONE || typeFrame == FRAME_RSP || typeFrame == FRAME_TOF_PRINT || typeFrame == FRAME_TOF_PWD || typeFrame == FRAME_TOF_PROCESSDATA
...
// 2 = 1+ 1 = (TOF) + C1H(EOF)
        ...
        if ((typeFrame == FRAME_NACK || typeFrame == FRAME_ACK || typeFrame == FRAME_ENQ || typeFrame == FRAME_EOT) && srcBuff[offset + 1] == B_EOF) {
        ...
        if ((typeFrame == FRAME_PHONE || typeFrame == FRAME_RSP || typeFrame == FRAME_MSR || typeFrame == FRAME_TOF_PWD || typeFrame == FRAME_TOF_PROCESSDATA || typeFrame == FRAME_STATUS_RESPONSE) && (offset + 4 + BYTES_OF_DATA_LENGTH) <= length) {// 4
            // =
            // 1+2+1
            // =
            // DATA_ID+CHECK_SUM+C1H(EOF)
            offset++; // (TOF)
            ...
            try {
                ...
                for (IMessageListener list : Common.msg_listeners.values())
                    list.exception("[exception in FrameUnpack in PocketPos][FrameUnpack][" + e.getLocalizedMessage() + "] ");
                dataLength = -1;
            }
